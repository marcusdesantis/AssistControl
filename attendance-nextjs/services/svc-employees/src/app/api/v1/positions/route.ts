...
export const POST = withPlanGate('organization', async (req: Request, { tenantId, admin }) => {
  ...
  createLog({ tenantId, userId: admin.sub, userName: admin.username, action: 'position.create', module: 'organization', detail: { name }, ip: getClientIp(req) })
  return apiOk(result, 'Cargo creado.')
})
