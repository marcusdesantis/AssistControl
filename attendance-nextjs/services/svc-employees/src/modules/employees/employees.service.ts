...
function toDto(emp: any) {
  return {
    ...
    scheduleStartDate:   emp.scheduleStartDate ?? null,
...
export async function create(tenantId: string, dto: CreateEmployeeDto) {
  ...
  const emp = await prisma.employee.create({
    data: {
      ...
      scheduleStartDate:   dto.scheduleStartDate ? new Date(dto.scheduleStartDate) : new Date(),
      worksOnHolidays:     dto.worksOnHolidays ?? true,
  ...
  const updated = await prisma.employee.update({
    ...
    data: {
      ...
      scheduleStartDate:   dto.scheduleStartDate
        ...
        : (dto.scheduleId !== emp.scheduleId ? new Date() : emp.scheduleStartDate),
      worksOnHolidays:     dto.worksOnHolidays ?? true,
      email:               dto.email.trim().toLowerCase(),
      phone:               dto.phone?.trim() ?? null,
      hireDate:            dto.hireDate,
