...
function ReportsPageInner() {
  ...
  return (
    <div className="flex flex-col -mt-6 -mb-6 md:h-[calc(100vh-3.5rem)]">
      ...
      {/* Body */}
      <div className="flex flex-col md:flex-row md:flex-1 md:overflow-hidden md:min-h-0 border-t border-gray-200">
        ...
        {/* Right panel */}
        <div className="flex-1 flex flex-col md:overflow-hidden md:min-h-0 relative">

          {/* Filters bar */}
          <div id="tour-rep-filters" className="bg-white border-b border-gray-200 px-4 py-2 sm:py-4 shrink-0">
            <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-end gap-2 sm:gap-3">
              {/* Desde / Hasta — misma fila 50/50 */}
              ...
              {/* Departamento + botón Buscar en la misma fila en móvil */}
              <div className="flex gap-2 sm:contents">
                <div className="flex flex-col gap-1 flex-1 sm:flex-none">
                ...
                <button onClick={handleGenerate} disabled={loading}
                  className="sm:hidden flex-none self-end px-4 py-1.5 bg-primary-600 hover:bg-primary-700 disabled:opacity-60 text-white text-sm font-medium rounded-lg transition-colors">
                  {loading ? '…' : 'Buscar'}
                </button>
              </div>
              {/* Buscar empleado */}
              ...
              {/* Botón Buscar — solo desktop */}
              <button onClick={handleGenerate} disabled={loading}
                className="hidden sm:block px-5 py-1.5 bg-primary-600 hover:bg-primary-700 disabled:opacity-60 text-white text-sm font-medium rounded-lg transition-colors">
                  ...
                  {pagedGroups.map((g, idx) => {
                    ...
                    return (
                      <div key={g.employeeCode}
                        className={`px-4 py-3 transition-colors ${checked ? 'bg-primary-50' : 'bg-white'}`}>
                        {/* Row 1: checkbox + name + eye */}
                        <div className="flex items-center gap-3">
                          <input type="checkbox" checked={checked}
                            onChange={() => toggleCode(g.employeeCode)}
                            className="w-4 h-4 rounded border-gray-300 text-primary-600 cursor-pointer shrink-0" />
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-semibold text-gray-800 truncate">{g.fullName}</p>
                            <p className="text-xs text-gray-400 truncate">
                              {g.employeeCode} · {g.department}
                            </p>
                          </div>
                          <button onClick={() => openViewer(g.employeeCode)}
                            className="p-2 rounded-lg bg-gray-100 hover:bg-primary-100 text-gray-500 hover:text-primary-600 transition-colors shrink-0"
                            title="Ver reporte">
                        ...
                        <div className="mt-2 ml-7 flex items-center gap-3 flex-wrap">
                          <span className="text-xs font-medium text-gray-500">{summaryLabel(selected)}:</span>
