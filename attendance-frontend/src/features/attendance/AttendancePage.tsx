    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <div className="py-12 text-center text-gray-400 text-sm">Sin resultados para el período seleccionado</div>
        ) : (
          <>
            {/* ── Mobile: cards ── */}
            <div className="sm:hidden divide-y divide-gray-100">
              {rows.map(row => (
                <div key={row.employeeId} className="p-4 space-y-2">
                  <div>
                    <p className="font-semibold text-gray-900">{row.fullName}</p>
                    <p className="text-xs text-gray-400 font-mono">{row.employeeCode} · {row.department}</p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-green-100 text-green-700">
                      ✓ {row.present} presentes
                    </span>
                    {row.late > 0 && (
              ...
              <div className="p-4 bg-gray-50">
                <p className="text-xs font-semibold text-gray-500 mb-2">Total página ({rows.length} empleados)</p>
                <div className="flex flex-wrap gap-2">
                  <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-green-100 text-green-700">✓ {totals.present}</span>
                  <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-yellow-100 text-yellow-700">⚠ {totals.late}</span>
                  <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-red-100 text-red-700">✗ {totals.absent}</span>
                  <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-gray-100 text-gray-600">⏱ {formatHours(totals.hours)}</span>
                </div>
              </div>
            </div>
                ...
                <tbody className="divide-y divide-gray-100">
                  {rows.map((row, idx) => (
                    <tr key={row.employeeId} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3 text-gray-400 text-xs tabular-nums">{(page - 1) * pageSize + idx + 1}</td>
                      ...
                      <td className="px-4 py-3"><span className="inline-flex px-2.5 py-1 rounded-full text-xs font-medium bg-green-100 text-green-700">{row.present}</span></td>
                      <td className="px-4 py-3">
                        {row.late > 0 ? <span className="inline-flex px-2.5 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-700">{row.late}</span> : <span className="text-gray-300">—</span>}
                      </td>
                      <td className="px-4 py-3">
                        {row.absent > 0 ? <span className="inline-flex px-2.5 py-1 rounded-full text-xs font-medium bg-red-100 text-red-700">{row.absent}</span> : <span className="text-gray-300">—</span>}
                      </td>
                      <td className="px-4 py-3 text-gray-600 font-mono text-xs">{formatHours(row.totalHours)}</td>
                    </tr>
                  ))}
                  <tr className="bg-gray-50 border-t-2 border-gray-200 font-semibold">
                    <td className="px-4 py-3 text-gray-700" colSpan={3}>Total página ({rows.length} empleados)</td>
                      ...
                      <div key={row.employeeId} className="p-4 space-y-3">
                        ...
                        <div className="flex items-start justify-between gap-2">
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-gray-400 font-mono mt-0.5">{row.employeeCode} · {row.department}</p>
                          </div>
                          <span className={`shrink-0 px-2.5 py-1 rounded-full text-xs font-medium ${STATUS_BADGE[row.statusKey] ?? STATUS_BADGE.None}`}>
                            {row.statusLabel}
                          </span>
                        </div>
                        ...
                        <div className="flex gap-3 text-xs">
                          <div className="flex items-center gap-1.5 text-green-700">
                            ...
                            <span className="font-mono">{fmt(row.checkInTime)}</span>
                          </div>
                          <div className="flex items-center gap-1.5 text-blue-700">
                            <LogOut className="w-3.5 h-3.5" />
                            <span className="font-mono">{fmt(row.checkOutTime)}</span>
                          </div>
                          {row.hoursWorked != null && row.hoursWorked > 0 && (
                            <div className="flex items-center gap-1 text-gray-500">
                              <Clock className="w-3.5 h-3.5" />
                              <span>{formatHours(row.hoursWorked)}</span>
                            </div>
                          )}
                        </div>

                        {/* Fila 3: origen + acciones */}
                        <div className="flex items-center justify-between gap-2">
                          {row.records.length > 0 ? (
                            <button onClick={() => setOriginsTarget(row)}
                              ...
                              {row.records.length > 1 ? `${row.records.length} registros` : (ORIGIN_LABEL[row.records[0].registeredFrom] ?? row.records[0].registeredFrom)}
                              {row.records.some(r => r.latitude != null) && <MapPin className="w-3 h-3 text-red-400" />}
                            </button>
                          ) : <div />}

                          <div className="flex items-center gap-1.5">
                            {canCheckIn && (
                              <button onClick={() => handleCheckIn(row)} disabled={isSaving}
                            ...
                            {canCheckOut && (
                              <button onClick={() => setCheckOutTarget(row)} disabled={isSaving}
                            ...
                            <button onClick={() => setHistoryTarget({ id: row.employeeId, employeeCode: row.employeeCode, fullName: row.fullName, departmentName: row.department } as unknown as Employee)}
                          ...
                          <tr key={row.employeeId} className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-4 py-3">
                              {row.records.length === 0 ? (
                                ...
                                <button onClick={() => setOriginsTarget(row)}
                                  className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-gray-200 hover:border-primary-300 hover:bg-primary-50 text-xs text-gray-600 hover:text-primary-700 transition-all">
                                  <OriginIcon from={row.records[0].registeredFrom} />
                                  {row.records.length > 1 ? <span>{row.records.length} registros</span> : <span>{ORIGIN_LABEL[row.records[0].registeredFrom] ?? row.records[0].registeredFrom}</span>}
                                  {row.records.some(r => r.latitude != null) && <MapPin className="w-3 h-3 text-red-400" />}
                                </button>
                              )}
                            </td>
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-1">
                                ...
                                {row.notes && <div title={row.notes} className="p-1.5 text-gray-400 cursor-help"><FileText className="w-3.5 h-3.5" /></div>}
                                <button onClick={() => setHistoryTarget({ id: row.employeeId, employeeCode: row.employeeCode, fullName: row.fullName, departmentName: row.department } as unknown as Employee)}
